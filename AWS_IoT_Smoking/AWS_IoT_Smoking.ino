...
Led led1(LED_1_PIN); //빨간색 led
Motor motor1(MOTOR_1_PIN); //환풍기 모터
int motor2 = 11; //환풍기 모터
...
void getDeviceStatus(char* payload) {
  ...
  float t = mq7.getPPM(); //CO값 받아오기


  // Read led status
  const char* led = (led1.getState() == LED_ON)? "ON" : "OFF";
  const char* motor = (motor1.getState() == MOTOR_ON)? "ON": "OFF";

  if(t>10.0){ //농도 높으면 led on, 환풍기 on
    led1.on();
    motor1.on();
  }
  else{
  ...
  sprintf(payload,"{\"state\":{\"reported\":{\"CO\":\"%0.2f\",\"LED\":\"%s\",\"MOTOR\":\"%s\"}}}",t,led,motor);
